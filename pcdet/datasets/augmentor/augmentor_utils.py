...
def global_scaling_with_roi_boxes_v2(gt_boxes, roi_boxes, gt_prev_boxes, points, scale_range, return_scale=False):
    """
    Args:
        gt_boxes: (N, 7), [x, y, z, dx, dy, dz, heading]
        points: (M, 3 + C),
    ...
    gt_boxes[:, :6] *= noise_scale
    roi_boxes[:,:, [0,1,2,3,4,5,7,8]] *= noise_scale
    gt_prev_boxes[:,:, [0,1,2,3,4,5,7,8]] *= noise_scale
    if return_scale:
        return gt_boxes,roi_boxes, gt_prev_boxes, points, noise_scale
    return gt_boxes, roi_boxes, gt_prev_boxes, points
