...
class DataAugmentor(object):
    ...
    def random_world_flip(self, data_dict=None, config=None):
        ...
        for cur_axis in config['ALONG_AXIS_LIST']:
            ...
            #TODO
            if 'gt_sequence_boxes' in data_dict.keys():
                num_frame, num_rois,dim = data_dict['gt_sequence_boxes'].shape
                roi_boxes, _, _ = getattr(augmentor_utils, 'random_flip_along_%s' % cur_axis)(
                data_dict['gt_sequence_boxes'].reshape(-1,dim), np.zeros([1,3]), return_flip=True, enable=enable
                )
                data_dict['gt_sequence_boxes'] = roi_boxes.reshape(num_frame, num_rois,dim)
    ...
    def random_world_rotation(self, data_dict=None, config=None):
        ...
        if 'gt_sequence_boxes' in data_dict.keys():
            num_frame, num_rois,dim = data_dict['gt_sequence_boxes'].shape
            roi_boxes, _, _ = augmentor_utils.global_rotation(
            data_dict['gt_sequence_boxes'].reshape(-1, dim), np.zeros([1, 3]), rot_range=rot_range, return_rot=True, noise_rotation=noise_rot)
            data_dict['gt_sequence_boxes'] = roi_boxes.reshape(num_frame, num_rois,dim)
    ...
    def random_world_scaling(self, data_dict=None, config=None):
        ...
        if 'roi_boxes' in data_dict.keys():
            ...
            if 'gt_sequence_boxes' in data_dict.keys():
                gt_boxes, roi_boxes, gt_prev_boxes, points, noise_scale = augmentor_utils.global_scaling_with_roi_boxes_v2(
                    data_dict['gt_boxes'], data_dict['roi_boxes'], data_dict['gt_sequence_boxes'], data_dict['points'], config['WORLD_SCALE_RANGE'], return_scale=True
                )
                data_dict['roi_boxes'] = roi_boxes
                data_dict['gt_sequence_boxes'] = gt_prev_boxes
            else:
