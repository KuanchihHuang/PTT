...
class WaymoDataset(DatasetTemplate):
    ...
    def get_sequence_data(self, info, points, sequence_name, sample_idx, sequence_cfg, load_pred_boxes=False, load_prev_gt_boxes=False):
        ...
        #TODO/#NOTE
        sample_idx_pre_list = np.clip(sample_idx_pre_list, 0, len(sequence_info)-1)



        gt_sequence_boxes_all = None
        #TODO: load_prev gt
        if load_prev_gt_boxes:
            annos = sequence_info[sample_idx]['annos']
            gt_boxes_lidar = annos['gt_boxes_lidar']
            mask = ((annos['num_points_in_gt'] > 0) & (annos['name'] != 'Sign'))
            names = annos['name'][mask]
            gt_boxes = gt_boxes_lidar[mask][:,:7]
            #labels = np.array([map_waymo_name_to_id[name] for name in names]).reshape(-1,1)
            labels = np.array([self.class_names.index(names[k]) + 1 for k in range(names.shape[0])]).reshape(-1,1)
            scores = np.ones(labels.shape).astype(np.float)
            gt_boxes = np.concatenate([gt_boxes,scores,labels], axis=-1)
            gt_sequence_boxes_all = [gt_boxes]
        ...
        for idx, sample_idx_pre in enumerate(sample_idx_pre_list):
            ...
            #TODO:
            if load_prev_gt_boxes:
                annos = sequence_info[sample_idx_pre]['annos']
                gt_boxes_lidar = annos['gt_boxes_lidar']
                mask = ((annos['num_points_in_gt'] > 0) & (annos['name'] != 'Sign'))
                names = annos['name'][mask]
                gt_boxes = gt_boxes_lidar[mask][:,:7]
                ...
                scores = np.ones(labels.shape).astype(np.float)
                gt_boxes = np.concatenate([gt_boxes,scores,labels], axis=-1)
                gt_boxes = self.transform_prebox_to_current(gt_boxes, pose_pre, pose_cur)
                gt_sequence_boxes_all.append(gt_boxes)

        if self.dataset_cfg.get('ONLY_CURRENT', False):
            pass
        else:
        ...
        if load_prev_gt_boxes:
            gt_sequence_boxes_all = self.reorder_rois_for_refining(gt_sequence_boxes_all)

        return points, num_points_all, sample_idx_pre_list, poses, pred_boxes, pred_scores, pred_labels, gt_sequence_boxes_all
    ...
    def __getitem__(self, index):
        ...
        if self.dataset_cfg.get('SEQUENCE_CONFIG', None) is not None and self.dataset_cfg.SEQUENCE_CONFIG.ENABLED:
            points, num_points_all, sample_idx_pre_list, poses, pred_boxes, pred_scores, pred_labels, gt_sequence_boxes = self.get_sequence_data(
                ...
                #load_prev_gt_boxes = True #Tmp
                load_prev_gt_boxes = False #Tmp
            )
...
def create_waymo_infos_v2(dataset_cfg, class_names, data_path, save_path,
...
def create_waymo_infos(dataset_cfg, class_names, data_path, save_path,
    ...
    train_filename = save_path / ('%s_infos_%s.pkl' % (processed_data_tag, train_split))
    val_filename = save_path / ('%s_infos_%s.pkl' % (processed_data_tag, val_split))
    test_filename = save_path / ('%s_infos_%s.pkl' % (processed_data_tag, val_split))
