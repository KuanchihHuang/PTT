...
class PTTransformer(nn.Module):
    def __init__(self, input_dim, joint_feat=False,model_cfg=None):
        ...
        self.long_frame = model_cfg.NUM_LONG_FRAME
        self.short_frame = model_cfg.NUM_SHORT_FRAME
        self.future_frame = model_cfg.NUM_FUTURE_FRAME
        ...
        self.x_bn2 = nn.BatchNorm1d(channels)

        self.fc_s1 = nn.Linear(channels, 256)
        self.fc_s2 = nn.Linear(256, 3, bias=False)
        self.fc_ce1 = nn.Linear(channels, 256)
        self.fc_ce2 = nn.Linear(256, 3, bias=False)
        self.fc_hr1 = nn.Linear(channels, 256)
        self.fc_hr2 = nn.Linear(256, 1, bias=False)
        self.output_channel = 512
        self.conv1 = torch.nn.Conv1d(input_dim, 64, 1)
        self.conv2 = torch.nn.Conv1d(96, 128, 1)
        self.conv2_p = torch.nn.Conv1d(256, 32, 1)
        
        self.conv2_long = torch.nn.Conv1d(128, 256, 1)
        self.bn2_long = nn.BatchNorm1d(256)
        self.conv2_short = torch.nn.Conv1d(128, 256, 1)
        self.bn2_short = nn.BatchNorm1d(256)
        self.conv2_future = torch.nn.Conv1d(128, 256, 1)
        self.bn2_future = nn.BatchNorm1d(256)
        
        self.conv_future = torch.nn.Conv1d(96, 64, 1)
        self.bn_future = nn.BatchNorm1d(64)

        self.conv_point = torch.nn.Conv1d(256 * 3, 256, 1)
        self.bn_point = nn.BatchNorm1d(256)

        self.conv_final = torch.nn.Conv1d(256 * 3, 512, 1)
        self.bn_final = nn.BatchNorm1d(512)

        self.conv3 = torch.nn.Conv1d(128 * 1, 256, 1)
        self.conv4 = torch.nn.Conv1d(256,  self.output_channel, 1)
        self.bn1 = nn.BatchNorm1d(64)
        self.bn2 = nn.BatchNorm1d(128)
        self.bn2_p = nn.BatchNorm1d(32)
        self.bn3 = nn.BatchNorm1d(256)
        self.bn4 = nn.BatchNorm1d(self.output_channel)
        self.attn1_long = Attention(dim_Q=96,dim_K=96, dim_LIN=96, num_heads=1)
        self.attn1_short = Attention(dim_Q=96,dim_K=96, dim_LIN=96, num_heads=1)
        self.attn1_future = Attention(dim_Q=96,dim_K=96, dim_LIN=96, num_heads=1)
        self.attn1 = Attention(dim_Q=64,dim_K=64, dim_LIN=64, num_heads=1)
        self.attn2 = Attention(dim_Q=128,dim_K=128, dim_LIN=128, num_heads=1)
        self.attn3 = Attention(dim_Q=256,dim_K=256, dim_LIN=256, num_heads=4)
        self.attn3_point = Attention(dim_Q=256,dim_K=256, dim_LIN=256, num_heads=4)
        self.cross_long = Attention(dim_Q=256,dim_K=256, dim_LIN=256, num_heads=4)
        self.cross_short = Attention(dim_Q=256,dim_K=256, dim_LIN=256, num_heads=4)
        self.cross_future = Attention(dim_Q=256,dim_K=256, dim_LIN=256, num_heads=4)

        self.future_token = nn.Parameter(torch.Tensor(1,1,32))
        nn.init.xavier_uniform_(self.future_token)

    def forward(self, x, feat=None, cur_feat=None):

        #x: sequential proposal feat
        #feat: sequential point feat
        #cur_feat: current point feat

        feat = torch.max(feat,2)[0]
        x = self.pre_bn(x)
        x = F.relu(self.bn1(self.conv1(x)))
        feat_new = F.relu(self.bn2_p(self.conv2_p(feat.permute(1,2,0))))
        x = torch.cat([x,feat_new], 1) #P-T feature

        x_long = x[...,self.future_frame+self.short_frame:]
        x_short = x[...,self.future_frame:self.future_frame+self.short_frame]
        x_future = x[...,:self.future_frame]
        x_long = x_long.permute(0,2,1)
        x_short = x_short.permute(0,2,1)

        x_future = F.relu(self.bn_future(self.conv_future(x_future)))
        x_future = torch.cat([self.future_token.repeat(x_future.shape[0], x_future.shape[2], 1).permute(0,2,1), x_future], 1)
        x_future = x_future.permute(0,2,1)

        x_long = self.attn1_long(Q=x_long,K=x_long)
        x_short = self.attn1_short(Q=x_short,K=x_long)
        x_sl = torch.cat([x_short,x_long],axis=1)
        x_future = self.attn1_future(Q=x_future,K=x_sl)
        x = torch.cat([x_future,x_sl],axis=1)
        x = x.permute(0,2,1)
        x = F.relu(self.bn2(self.conv2(x)))
        x = x.permute(0,2,1)
        x = self.attn2(Q=x,K=x).permute(0,2,1) #Enhanced P-T feature

        #TODO: Point: G^T
        N, point_N, _ = cur_feat.shape
        cur_feat = self.attn3_point(Q=cur_feat,K=cur_feat)
        cur_feat = torch.max(cur_feat, 1)[0]

        x_long = x[...,self.future_frame+self.short_frame:]
        x_short = x[...,self.future_frame:self.future_frame+self.short_frame]
        x_future = x[...,:self.future_frame]
        x_long = F.relu(self.bn2_long(self.conv2_long(x_long)))
        x_short = F.relu(self.bn2_short(self.conv2_short(x_short)))
        x_future = F.relu(self.bn2_future(self.conv2_future(x_future)))

        feat_long = self.cross_long(Q=cur_feat.unsqueeze(1),K=x_long.permute(0,2,1))
        feat_short = self.cross_short(Q=cur_feat.unsqueeze(1),K=x_short.permute(0,2,1))
        feat_future = self.cross_future(Q=cur_feat.unsqueeze(1),K=x_future.permute(0,2,1))

        feat_cross = torch.cat([feat_future, feat_short, feat_long], axis=-1).permute(0,2,1)  

        feat_cross = F.relu(self.bn_point(self.conv_point(feat_cross)))
        
        cur_feat = feat_cross.squeeze(-1) + cur_feat

        x_ori = F.relu(self.bn3(self.conv3(x)))
        x = self.bn4(self.conv4(x_ori)) 
        x = torch.max(x, 2, keepdim=True)[0] 
        x = torch.cat([x, cur_feat.unsqueeze(-1)], axis=1)
        ...
        return torch.cat([centers, sizes, headings],-1),feat,feat_traj, cur_feat
...
class PTTHead(RoIHeadTemplate):
    def __init__(self,model_cfg, num_class=1,**kwargs):
        ...
        self.num_proxy_points = 64

        self.ptt = PTTransformer(8,model_cfg=self.model_cfg)
        ...
        self.jointembed = MLP(self.hidden_dim*(1+1), model_cfg.Transformer.hidden_dim, self.box_coder.code_size * self.num_class, 4)
        ...
        self.up_dimension_motion = MLP(input_dim = 30, hidden_dim = 64, output_dim = hidden_dim//num_radius, num_layers = 3)
        ...
        if model_cfg.get('USE_FUTURE_FRAME', None):
            num_future_frame = model_cfg.get('NUM_FUTURE_FRAME', 16)
        else:
            num_future_frame = 0

        cross_frame = model_cfg.Transformer.num_frames + num_future_frame - 1
    ...
    def crop_current_frame_points(self, src, batch_size,trajectory_rois,num_rois,batch_dict):
        ...
        for bs_idx in range(batch_size):
            ...
            #TODO: Real current frame
            if self.model_cfg.get('NO_ACCUMULATE',None):
                cur_points = cur_points[cur_points[:,-1] == 0.0]
    ...
    def get_proposal_aware_geometry_feature_src(self,src, batch_size,trajectory_rois,num_rois,batch_dict):
        ...
        proxy_point_geometry = src_gemoetry
        proxy_points = None
    ...
    def trajectories_auxiliary_branch_v2(self,trajectory_rois):
    ...
    def forward(self, batch_dict):
        ...
        ori_trajectory_rois = trajectory_rois.clone()
        num_real_future = self.model_cfg.get('NUM_REAL_FUTURE', 0)
        trajectory_rois = trajectory_rois[:,num_real_future].unsqueeze(1)
        tmp_num_frames = copy.deepcopy(batch_dict['num_frames'])
        batch_dict['num_frames'] = 1

        #Sampling points
        sampling_type = 'msf' #or 'mppnet'

        if sampling_type == 'msf':
        ...
        if self.model_cfg.get('USE_FUTURE_FRAME', None):
            num_future_frame = self.model_cfg.get('NUM_FUTURE_FRAME', 16)
        else:
            num_future_frame = 0
            
        src = src.unsqueeze(2).expand(-1,-1,tmp_num_frames+num_future_frame,-1,-1)
        src = src.reshape(batch_size, num_rois, (tmp_num_frames+num_future_frame)*num_sample,5)

        batch_dict['num_frames'] = tmp_num_frames
        trajectory_rois = ori_trajectory_rois

        src = src.view(batch_size * num_rois, -1, src.shape[-1]) 
        
        num_real_future = self.model_cfg.get('NUM_REAL_FUTURE', 0)
        gt_traj = trajectory_rois[:,:num_real_future]
        trajectory_rois = trajectory_rois[:,num_real_future:]

        num_future_frame = 0

        if self.model_cfg.get('USE_FUTURE_FRAME', None):
            num_future_frame = self.model_cfg.get('NUM_FUTURE_FRAME', 16)
            cur_batch_boxes = trajectory_rois[:,0]
            new_trajectory_rois = []

            #zlwh = trajectory_rois[:,:,:,2:6] #4,64,96,4
            #b,f,num_obj,_ = zlwh.shape

            #select_zlwh = zlwh[:,np.random.choice(f,5),:,:]
            #select_zlwh = torch.mean(select_zlwh, axis=1)

            for i in range(1, num_future_frame+1):
                frame = torch.zeros_like(cur_batch_boxes) #batch,96,9
                frame[:,:,0:2] = cur_batch_boxes[:,:,0:2] - i * cur_batch_boxes[:,:,7:9]
                
                #TODO: aug pos##########
                #pos_shift = (torch.rand((frame.shape[0], frame.shape[1], 2), device=frame.device) - 0.5)
                #frame[:,:,0:2] += pos_shift
                ########################

                frame[:,:,2:] = cur_batch_boxes[:,:,2:].detach().clone()
                
                #TODO:heading
                #frame[:,:,6] = frame[:,:,6] + i * (trajectory_rois[:,0,:,6] - trajectory_rois[:,1,:,6])
                #print(frame[:,:,2:6].shape, select_zlwh.shape)
                #frame[:,:,2:6] = select_zlwh.detach().clone()

                #new_trajectory_rois.append(new_frame.unsqueeze(1))
                new_trajectory_rois.append(frame.unsqueeze(1))
            new_trajectory_rois = torch.flip(torch.cat(new_trajectory_rois, axis=1), dims=(1,))
            trajectory_rois = torch.cat([new_trajectory_rois, trajectory_rois], axis=1)
        
        ##########################

        src_geometry_feature, proxy_points = self.get_proposal_aware_geometry_feature_src(src,batch_size,trajectory_rois,num_rois,batch_dict)
        ...
        if True:
            src = src.view(src.shape[0], -1, src.shape[-1]*2) #266,256,256
            feat_resize= self.num_lidar_points // 2
            src = torch.stack([src[:,i*feat_resize:(i+1)*feat_resize] for i in range(batch_dict['num_frames']+num_future_frame)], 0)

            if True:
                if self.model_cfg.get('USE_FUTURE_FRAME', None):
                    num_future_frame = self.model_cfg.get('NUM_FUTURE_FRAME', 16)# + 1
                else:
                    num_future_frame = 0
                cur_src = src[num_future_frame]#.unsqueeze(0) #1*133*256

            box_seq = self.trajectories_auxiliary_branch_v2(trajectory_rois)
            batch_rcnn = box_seq.shape[0]*box_seq.shape[2]
            box_reg, feat_box, traj_feat, cur_src =  self.ptt(box_seq.permute(0,2,3,1).contiguous().view(batch_rcnn,box_seq.shape[-1],box_seq.shape[1]), src, cur_src)
            traj_feat = traj_feat.permute(2,0,1)
            src = cur_src.unsqueeze(0)
    ...
    def get_loss(self, tb_dict=None):
        ...
        #rcnn_loss += self.forward_ret_dict['pred_loss'] #TODO
